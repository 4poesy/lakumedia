...
/**
 * Real-Time Global Match Scoreboard Fetcher for Yesterday, Today, and Tomorrow
 * Fetches 100% real matches dynamically from ESPN Live API across 21 global leagues.
 */
export async function fetchLiveScoreboardForDateOffset(offset: 'yesterday' | 'today' | 'tomorrow'): Promise<ApiMatchFixture[]> {
  ...
  // Query ESPN live scoreboards concurrently across top global leagues
  const promises = LEAGUES_TO_FETCH.map(async (lg) => {
    try {
      const url = `https://site.web.api.espn.com/apis/site/v2/sports/soccer/${lg.code}/scoreboard?dates=${dateStr}`;
