...
import { ChevronLeft, ChevronRight, Calendar, Flame, ArrowRight } from 'lucide-react';
...
export function HeroSlider({ slides }: HeroSliderProps) {
  ...
  return (
    <div
      className="relative w-full h-[400px] sm:h-[480px] lg:h-[520px] rounded-3xl overflow-hidden shadow-lg border border-slate-200 bg-slate-900 group my-4"
    ...
    >
      ...
      <div className="relative w-full h-full">
        <Image
          ...
          priority
        />
        {/* Solid Sharp Dark Overlay Box for Text Readability */}
        <div className="absolute inset-0 bg-slate-950/40" />
      </div>

      {/* Content Overlay Box */}
      <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 space-y-4 max-w-4xl bg-slate-950/90 text-white border-t border-slate-800">
        <div className="flex items-center space-x-3">
          ...
          <span className="px-2 py-0.5 text-[10px] font-mono font-bold bg-[#2A2E7F] text-white rounded">
            8K ULTRA HD
          </span>
        </div>

        <h2 className="text-xl sm:text-3xl lg:text-4xl font-extrabold text-white leading-tight hover:text-[#D9541E] transition-colors">
          <Link href={`/article/${currentSlide.slug}`}>
            {currentSlide.title}
          </Link>
        </h2>

        <p className="text-xs sm:text-sm text-slate-200 line-clamp-2 leading-relaxed font-medium max-w-3xl hidden sm:block">
        ...
        <div className="pt-2 flex items-center justify-between">
          <Link
            ...
            className="px-5 py-2.5 rounded-xl bg-[#D9541E] hover:bg-[#b84315] text-white font-extrabold text-xs flex items-center gap-2 shadow-sm transition-colors"
          >
      ...
      <button
        onClick={handlePrev}
        className="absolute left-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-slate-900/80 hover:bg-[#2A2E7F] text-white flex items-center justify-center shadow-md transition-colors border border-slate-700 z-10"
      ...
      <button
        onClick={handleNext}
        className="absolute right-4 top-1/2 -translate-y-1/2 w-11 h-11 rounded-full bg-slate-900/80 hover:bg-[#2A2E7F] text-white flex items-center justify-center shadow-md transition-colors border border-slate-700 z-10"
