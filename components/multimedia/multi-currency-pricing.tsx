...
export function MultiCurrencyPricing() {
  ...
  return (
    <section className="space-y-8">
      ...
      <ScrollRevealSection className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-slate-950 p-4 sm:p-6 rounded-3xl border border-slate-800 shadow-xl max-w-7xl mx-auto">
      ...
      {/* Pricing Cards Grid */}
      <ScrollRevealSection stagger className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {rawPackages.map((pkg) => (
          <ScrollRevealChild key={pkg.id}>
            <CinematicCardMotion hoverScale={1.025} hoverY={-4}>
              <div
                className={`rounded-3xl p-8 space-y-6 border transition-colors duration-300 flex flex-col justify-between h-full relative ${
                  pkg.popular
                    ? 'bg-slate-950 border-2 border-[#D9541E] shadow-2xl'
