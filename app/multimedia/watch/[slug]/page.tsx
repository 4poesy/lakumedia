...
import { ArrowLeft, Play, Clock, ShieldCheck, Film, Radio, Tv, Share2 } from 'lucide-react';
...
export default async function WatchPage({ params }: WatchPageProps) {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 theme-multimedia">
      ...
      <div className="relative w-full aspect-video rounded-3xl overflow-hidden bg-slate-900 border border-slate-200 shadow-xl flex flex-col justify-center items-center group">
      ...
      {/* Video Details */}
      <div className="space-y-4 bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="flex items-center space-x-3 text-xs">
              <span className="px-3 py-1 font-extrabold uppercase tracking-wider rounded-md bg-[#D9541E] text-white shadow-sm">
                {item.media_genres?.name || 'Entertainment'}
              </span>
              {item.is_kid_safe && (
                <span className="px-3 py-1 font-bold rounded-md bg-emerald-600 text-white flex items-center gap-1 shadow-sm">
            ...
            {item.season_number && item.episode_number && (
              <p className="text-xs font-extrabold text-[#D9541E] flex items-center gap-1.5 pt-1">
                <Tv className="w-4 h-4" /> Season {item.season_number} • Episode {item.episode_number}
              </p>
            )}
          </div>

          <button className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-800 text-xs font-bold flex items-center gap-1.5 border border-slate-200 transition-colors">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {episodes.map((ep: any) => (
              <Link
                ...
                className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm hover:border-[#D9541E]/50 transition-all flex space-x-4 group"
              >
                ...
                <div className="space-y-1 flex-1">
                  <span className="text-[10px] font-extrabold text-[#D9541E] uppercase tracking-wider block">
                    Season {ep.season_number || 1} • Episode {ep.episode_number || 1}
                  </span>
                  <h4 className="text-sm font-bold text-slate-900 group-hover:text-[#D9541E] transition-colors line-clamp-1">
                    {ep.title}
                  </h4>
